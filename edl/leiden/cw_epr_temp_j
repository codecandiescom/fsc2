...
sub field_check {
	...
	if ( $_[ 0 ] =~ /^[+-]?\.?$/ or
		 ( is_float1( $_[ 0 ] ) and $_[ 0 ] >= 0 and $_[ 0 ] <= 120019 ) ) {
...
sub end_field_check {
	...
	if ( $_[ 0 ] =~ /^[+-]?\.?$/ or
		 ( is_float2( $_[ 0 ] ) and $_[ 0 ] >= 0 and $_[ 0 ] <= 120019 ) ) {
...
";
	...
	FOR I = 1 : Num_Points {
        ...
        temp[ J, I ] = temp_contr_temperature( );
		output_value( B2, start_field + ( I - 1 ) * step_size );
		output_value( B3, temp[ J, I ] );
...
IF J > 0 {
    File3 = clone_file( File1, \"avg\", \"temp\" );
...
";
	...
	if ( $start_field < $end_field ) {
		print F "
				...
				fsave( File3, \"#\", start_field + ( I - 1 ) * step_size );";
	} else {
		print F "
				fsave( File2, \"#\", end_field - ( I - 1 ) * step_size );
				fsave( File3, \"#\", end_field - ( I - 1 ) * step_size );";
	}
	print F "
				FOR K = 1 : J {
					fsave( File2, \",#\", data[ K, I ] );
					fsave( File3, \",#\", temp[ K, I ] );
				}
				fsave( File2, \"\\n\" );
				fsave( File3, \"\\n\" );
			}
