...
/* digitizer, which is the time base divided by TDS520C_POINTS_PER_DIV. */
...
static void tds520c_window_check_2( void )
{
	...
	for ( w = tds520c.w; w != NULL; w = w->next )
	{
		...
		cs = lrnd( TDS520C_POINTS_PER_DIV * dcs );
		tb = lrnd( dtb );
		...
		cd = lrnd( TDS520C_POINTS_PER_DIV * dcd );
		tb = lrnd( dtb );
...
static void tds520c_window_check_3( void )
{
    ...
    window = tds520c.timebase * tds520c.rec_len / TDS520C_POINTS_PER_DIV;
    ...
    for ( w = tds520c.w; w != NULL; w = w->next )
    {
		...
		w->start_num = lrnd( ( w->start + tds520c.trig_pos * window )
							 * TDS520C_POINTS_PER_DIV / tds520c.timebase ) + 1;
	...
	if ( dir == GENERAL_TO_TDS520C )
	{
		switch ( channel )
		{
			...
			case DIGITIZER_CHANNEL_MATH1 :
				return TDS520C_MATH1;

			case DIGITIZER_CHANNEL_MATH2 :
				return TDS520C_MATH2;

			case DIGITIZER_CHANNEL_MATH3 :
				return TDS520C_MATH3;
