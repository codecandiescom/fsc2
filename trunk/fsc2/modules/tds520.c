...
Var *digitizer_timebase( Var *v )
{
	...
	if ( v == NULL )
	{
		...
		else if ( I_am == PARENT )
		{
			...
			eprint( FATAL, "%s:%ld: %s: Function %s with no argument can only "
...
Var *digitizer_num_averages( Var *v )
{
	...
	{
		eprint( FATAL, "%s:%ld: %s: Negative number of averages (%ld) in "
...
static Var *get_amplitude( Var *v, bool use_cursor )
{
	...
	if ( v == NULL )
	{
		eprint( FATAL, "%s:%ld: %s: Missing arguments in call of %s().\n",
				Fname, Lc, DEVICE_NAME, Cur_Func );
		THROW( EXCEPTION );
	}
...
Var *digitizer_lock_keyboard( Var *v )
{
	...
	else
	{
		...
		if ( v->type == INT_VAR )
			lock = v->val.lval == 0 ? SET : UNSET;
		else if ( v->type == FLOAT_VAR )
			lock = v->val.dval == 0.0 ? SET : UNSET;
		else
		{
