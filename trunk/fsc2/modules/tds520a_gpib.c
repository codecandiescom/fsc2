...
bool tds520a_init( const char *name )
{
    ...
    if ( gpib_write( tds520a.device, "DAT:ENC SRI;WID 2\n", 18 )
		 == FAILURE )
	{
		gpib_local( tds520a.device );
        return FAIL;
...
{
	print( FATAL, "%s: Communication with device failed.\n" );
	THROW( EXCEPTION );
}
...
bool tds520a_display_channel( int channel )
{
	...
	fsc2_assert( channel >= TDS520A_CH1 && channel < TDS520A_AUX1 );
...
double tds520a_get_sens( int channel )
{
	...
	fsc2_assert( channel >= TDS520A_CH1 && channel <= TDS520A_CH2 );
    ...
    reply[ length - 1 ] = '\0';
	tds520a.sens[ channel ] = T_atod( reply );
...
static double tds520a_get_area_wo_cursor( int channel, WINDOW *w )
{
	...
	return ( area - length * pos ) * tds520a.timebase / TDS520A_POINTS_PER_DIV;
}
						...
						bool use_cursor)
{
	...
	if ( channel >= TDS520A_CH1 && channel <= TDS520A_CH2 &&
		 ( ! tds520a.is_sens[ channel ] || ! tds520a.lock_state ) )
	{
		tds520a_get_sens( channel );
...
bool tds520a_lock_state( bool lock )
{
	...
	if ( ! tds520a.lock_state && lock )
		for ( channel = TDS520A_CH1; channel <= TDS520A_CH2; channel++ )
		{
			tds520a_get_sens( channel );
			tds520a.is_sens[ channel ] = SET;
		}
