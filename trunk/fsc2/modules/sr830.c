...
static double sr830_set_ref_freq( double freq );
static long sr830_get_ref_mode( void );
...
Var *lockin_phase( Var *v )
{
	...
	{
		if ( I_am == CHILD )         /* if called in EXPERIMENT section */
			return vars_push( FLOAT_VAR, sr830_set_phase( phase ) );
		else                         /* if called in a preparation sections */ 
		{
			sr830.phase = phase;
			sr830.P = SET;
...
Var *lockin_ref_freq( Var *v )
{
	...
	if ( sr830_get_ref_mode( ) != REF_MODE_INTERNAL )
	{
	...
	if ( I_am == CHILD )                /* if called in EXPERIMENT section */
		return vars_push( FLOAT_VAR, sr830_set_ref_freq( freq ) );
...
Var *lockin_ref_level( Var *v )
{
	...
	if ( v == NULL )
	{
		...
		else
		{
			...
			return vars_push( FLOAT_VAR, sr830_get_ref_level( ) );
		}
	}
	...
	if ( I_am == CHILD )                /* if called in EXPERIMENT section */
		return vars_push( FLOAT_VAR, sr830_set_ref_level( level ) );
	else                                /* if called in preparation sections */
	{
...
static bool sr830_init( const char *name )
{
	...
	if ( gpib_write( sr830.device, "OUTX 1", 6 ) == FAILURE ||
		 gpib_write( sr830.device, "*CLS", 4 )   == FAILURE ||
		 gpib_write( sr830.device, "OVRM 0", 6 ) == FAILURE )
		return FAIL;
	   
	/* Ask lock-in to send the error status byte and test if it does */

	length = 20;
	if ( gpib_write( sr830.device, "ERRS?", 5 ) == FAILURE ||
...
static double sr830_get_data( void )
{
	...
	if ( gpib_write( sr830.device, buffer, 6 ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static double sr830_get_sens( void )
{
	...
	if ( gpib_write( sr830.device, "SENS?", 5 ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static double sr830_get_tc( void )
{
	...
	if ( gpib_write( sr830.device, "OFLT?", 5 ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static double sr830_get_phase( void )
{
	...
	if ( gpib_write( sr830.device, "PHAS?", 5L ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static double sr830_get_ref_freq( void )
{
	...
	if ( gpib_write( sr830.device, "FREQ?", 5L ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static long sr830_get_ref_mode( void )
{
	...
	if ( gpib_write( sr830.device, "FMOD?", 5L ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static long sr830_get_harmonic( void )
{
	...
	if ( gpib_write( sr830.device, "HARM?", 5L ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
...
static double sr830_get_ref_level( void )
{
	...
	if ( gpib_write( sr830.device, "SLVL?", 5L ) == FAILURE ||
		 gpib_read( sr830.device, buffer, &length ) == FAILURE )
	{
		eprint( FATAL, "%s: Can't access the lock-in amplifier.\n",
				DEVICE_NAME );
