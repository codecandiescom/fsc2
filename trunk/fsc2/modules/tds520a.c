...
static struct {
	...
	bool is_sens[ TDS520A_CH2 + 1 ];
	double sens[ TDS520A_CH2 + 1 ];
} tds520a_store;
...
int tds520a_exp_hook( void )
{
	...
	tds520a_store.timebase       = tds520a.is_timebase;
...
int tds520a_end_of_exp_hook( void )
{
	...
	tds520a.is_timebase          = tds520a_store.timebase;
...
Var *digitizer_timebase( Var *v )
{
	...
	if ( timebase <= 0 )
	{
		eprint( FATAL, SET, "%s: Invalid zero or negative time base: %s.\n",
				DEVICE_NAME, tds520a_ptime( timebase ) );
		THROW( EXCEPTION );
	}
		 ...
		 fabs( timebase - tb[ TB ] ) > timebase * 1.0e-2 )  /* error > 1% ?  */
	{
		...
		eprint( WARN, SET, "%s: Can't set timebase to %s, using %s "
				"instead.\n", DEVICE_NAME, t, tds520a_ptime( tb[ TB ] ) );
		T_free( t );
	}
	...
	if ( TB < 0 )                                   /* not found yet ? */
	{
		...
		if ( timebase < tb[ 0 ] )
		{
			...
			eprint( WARN, SET, "%s: Timebase of %s is too low, using %s "
					"instead.\n", DEVICE_NAME, t, tds520a_ptime( timebase ) );
		}
		else
		{
		    timebase = tb[ 31 ];
...
Var *digitizer_trigger_channel( Var *v )
{
	if ( v == NULL )
	{
		if ( TEST_RUN )
		{
			if ( tds520a.is_trigger_channel )
				...
				return vars_push( INT_VAR, TDS520A_TEST_TRIG_CHANNEL );
		}
		else if ( I_am == PARENT )
		{
			if ( tds520a.is_trigger_channel )
	...
	{
		eprint( FATAL, SET, "%s: Invalid trigger channel name.\n",
				DEVICE_NAME );
		THROW( EXCEPTION );
	}
...
static Var *get_amplitude( Var *v, bool use_cursor )
{
	...
	if ( ch > TDS520A_REF4 )
	{
		eprint( FATAL, SET, "%s: Invalid channel specificationin %s().\n",
				DEVICE_NAME, Cur_Func );
		THROW( EXCEPTION );
	}
	...
	if ( v != NULL )
	{
		...
		if ( ( w = tds520a.w ) == NULL )
		{
			eprint( FATAL, SET, "No measurement windows have been "
					"defined for %s().\n", DEVICE_NAME, Cur_Func );
			THROW( EXCEPTION );
		}
