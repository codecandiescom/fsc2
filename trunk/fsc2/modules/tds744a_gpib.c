...
bool tds744a_init( const char *name )
{
	...
	if ( cp1 > cp2 )
	{
		...
		tds744a.cursor_pos = cp2;
	}
	else
		tds744a.cursor_pos = cp1;
...
void tds744a_finished( void )
{
	...
	if ( ! tds744a.is_reacting )
		return;

    gpib_write( tds744a.device, "ACQ:STATE STOP\n", 15 );
    tds744a_clear_SESR( );
    gpib_write( tds744a.device, cmd, strlen( cmd ) );
	gpib_local( tds744a.device );
...
bool tds744a_display_channel( int channel )
{
	...
	fsc2_assert( channel >= TDS744A_CH1 && channel < TDS744A_AUX );
...
double tds744a_get_sens( int channel )
{
	...
	fsc2_assert( channel >= TDS744A_CH1 && channel <= TDS744A_CH4 );
    ...
    reply[ length - 1 ] = '\0';
	tds744a.sens[ channel ] = T_atod( reply );
...
static double tds744a_get_area_wo_cursor( int channel, WINDOW *w )
{
	...
	return ( area - length * pos ) * tds744a.timebase / TDS744A_POINTS_PER_DIV;
}
						...
						bool use_cursor )
{
	...
	if ( channel >= TDS744A_CH1 && channel <= TDS744A_CH4 &&
		 ( ! tds744a.is_sens[ channel ] || ! tds744a.lock_state ) )
	{
		tds744a_get_sens( channel );
		tds744a.is_sens[ channel ] = SET;
	}
	...
	*length = w != NULL ? w->end_num - w->start_num : tds744a.rec_len;
	len = 2 * *length;
...
bool tds744a_lock_state( bool lock )
{
	...
	if ( ! tds744a.lock_state && lock )
		for ( channel = TDS744A_CH1; channel <= TDS744A_CH4; channel++ )
		{
			tds744a_get_sens( channel );
			tds744a.is_sens[ channel ] = SET;
		}
