...
FILE *hp8648b_find_table( char **name )
{
	...
	if ( ( tfp = hp8648b_open_table( *name ) ) != NULL )
	{
		hp8648b.table_file = T_strdup( *name );
		return tfp;
	}
...
double hp8648b_get_att_from_table( double freq )
{
	...
	if ( freq < hp8648b.min_table_freq )
	{
		print( WARN, "Frequency of %f MHz not covered by table, "
			   "interpolating.\n", freq * 1.0e-6 );
		return hp8648b.att_table[ 0 ].att;
	}

	if ( freq > hp8648b.max_table_freq )
	{
		print( WARN, "Frequency of %f MHz not covered by table, "
			   "interpolating.\n", freq * 1.0e-6 );
		return hp8648b.att_table[ hp8648b.att_table_len - 1 ].att;
	}
	...
	i_low = 0;
	i_high = hp8648b.att_table_len - 1;

	if ( freq == hp8648b.att_table[ i_low ].freq )
		return hp8648b.att_table[ i_low ].att;
	if ( freq == hp8648b.att_table[ i_high ].freq )
		return hp8648b.att_table[ i_high ].att;

	i_cur = lrnd( floor( ( i_high - i_low ) *
						 ( freq - hp8648b.att_table[ i_low ].freq ) /
						 ( hp8648b.att_table[ i_high ].freq
						   - hp8648b.att_table[ i_low ].freq ) ) ) + i_low;

	if ( freq > hp8648b.att_table[ i_cur ].freq )
	{
		i_low = i_cur;
		if ( freq < hp8648b.att_table[ i_cur + 1 ].freq )
			i_high = i_cur + 1;
	}
	else
		i_high = i_cur;
	...
	att =  ( hp8648b.att_table[ i_high ].att - hp8648b.att_table[ i_low ].att )
		 * ( freq - hp8648b.att_table[ i_low ].freq )
		 / (   hp8648b.att_table[ i_high ].freq
			 - hp8648b.att_table[ i_low ].freq )
		 + hp8648b.att_table[ i_low ].att;
...
double hp8648b_get_att( double freq )
{
	...
	if ( ! hp8648b.use_table )
		return hp8648b.attenuation;

	att =   hp8648b.attenuation + hp8648b_get_att_from_table( freq )
	...
	if ( att > hp8648b.min_attenuation )
	{
		print( SEVERE, "Attenuation dynamic range is insufficient (f = "
			   "%g MHz), using %f db instead of %f db.\n",
			   freq * 1.0e-6, hp8648b.min_attenuation, att );
