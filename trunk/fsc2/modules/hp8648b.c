...
int hp8648b_init_hook( void )
{
	...
	hp8648b.freq_is_set = UNSET;
	hp8648b.step_freq_is_set = UNSET;
	hp8648b.start_freq_is_set = UNSET;
	hp8648b.attenuation_is_set = UNSET;
	hp8648b.min_attenuation = MIN_ATTEN;

	hp8648b.table_file = NULL;
	hp8648b.use_table = UNSET;
	hp8648b.att_table = NULL;
	hp8648b.att_table_len = 0;
	hp8648b.real_attenuation = MAX_ATTEN - 100.0;  /* invalid value ! */

	hp8648b.att_ref_freq = DEF_ATT_REF_FREQ;

	hp8648b.mod_type = UNDEFINED;
	hp8648b.mod_type_is_set = UNSET;
	for ( i = 0; i < NUM_MOD_TYPES; i++ )
	{
		hp8648b.mod_source_is_set[ i ] = UNSET;
		hp8648b.mod_ampl_is_set[ i ] = UNSET;
	}
...
int hp8648b_exp_hook( void )
{
	...
	hp8648b = hp8648b_backup;

	if ( ! hp8648b_init( DEVICE_NAME ) )
	{
...
{
	if ( hp8648b.table_file != NULL )
		hp8648b.table_file = CHAR_P T_free( hp8648b.table_file );

	if ( hp8648b.use_table && hp8648b.att_table != NULL )
		hp8648b.att_table = ATT_TABLE_ENTRY_P T_free( hp8648b.att_table );
...
Var *synthesizer_state( Var *v )
{
	...
	if ( v == NULL )              /* i.e. return the current state */
		switch( FSC2_MODE )
		{
			...
			case TEST :
				return vars_push( INT_VAR, ( long ) hp8648b.state );

			case EXPERIMENT :
				return vars_push( INT_VAR,
								  ( long ) ( hp8648b.state =
											 hp8648b_get_output_state( ) ) );
		}
	...
	return vars_push( INT_VAR, ( long ) hp8648b_set_output_state( state ) );
}
...
Var *synthesizer_frequency( Var *v )
{
	...
	if ( v == NULL )              /* i.e. return the current frequency */
	{
		if ( FSC2_MODE != EXPERIMENT )
		{
			if ( ! hp8648b.freq_is_set )
			{
				...
				return vars_push( FLOAT_VAR, hp8648b.freq );
		}
	...
	{
		case PREPARATION :
			hp8648b.freq = hp8648b.start_freq = freq;
			hp8648b.freq_is_set = SET;
			hp8648b.start_freq_is_set = SET;
			break;

		case TEST :
			hp8648b.freq = freq;
			hp8648b.freq_is_set = SET;
			if ( ! hp8648b.start_freq_is_set )
			{
				hp8648b.start_freq = freq;
				hp8648b.start_freq_is_set = SET;
			}
			...
			if ( hp8648b.use_table )
				hp8648b.real_attenuation = hp8648b_get_att( freq );
			break;
		...
		case EXPERIMENT :
			...
			if ( hp8648b.use_table )
			{
				att = hp8648b_get_att( freq );
				if ( att != hp8648b.real_attenuation )
				{
					hp8648b_set_attenuation( att );
					hp8648b.real_attenuation = att;
				}
			}

			/* Finally set the frequency */

			hp8648b.freq = hp8648b_set_frequency( freq );
	}
...
Var *synthesizer_attenuation( Var *v )
{
	...
	if ( v == NULL )              /* i.e. return the current attenuation */
	{
		if ( FSC2_MODE != EXPERIMENT )
		{
			if ( ! hp8648b.attenuation_is_set )
			{
				...
				return vars_push( FLOAT_VAR, hp8648b.attenuation );
		}

		hp8648b.attenuation = MAX_ATTEN -100.0;
		return vars_push( FLOAT_VAR, hp8648b.attenuation );
	}
	...
	if ( att > hp8648b.min_attenuation || att < MAX_ATTEN )
	{
		print( FATAL, "RF attenuation (%g db) not within valid range (%g db "
			   "to %g db).\n", att, MAX_ATTEN, hp8648b.min_attenuation );
		if ( FSC2_MODE == EXPERIMENT )
			return vars_push( FLOAT_VAR, hp8648b.attenuation );
		else
			THROW( EXCEPTION );
	...
	switch ( FSC2_MODE )
	{
		case PREPARATION :
			...
			hp8648b.attenuation = att;
			hp8648b.attenuation_is_set = SET;
			break;

		case TEST :
			hp8648b.attenuation = hp8648b.real_attenuation = att;
			hp8648b.attenuation_is_set = SET;
			break;

		case EXPERIMENT :
			hp8648b.attenuation = hp8648b.real_attenuation =
				                                hp8648b_set_attenuation( att );
			break;
	}
...
Var *synthesizer_minimum_attenuation( Var *v )
{
	...
	if ( v == NULL )          /* i.e. return the current minimum attenuation */
		return vars_push( FLOAT_VAR, hp8648b.min_attenuation );
...
Var *synthesizer_step_frequency( Var *v )
{
	if ( v != NULL )
	{
		...
		if ( FSC2_MODE == PREPARATION && hp8648b.step_freq_is_set )
		{
			print( SEVERE, "RF step frequency has already been set to %f MHz, "
				   "keeping old value.\n", 1.0e-6 * hp8648b.step_freq );
			return vars_push( FLOAT_VAR, hp8648b.step_freq );
		}

		hp8648b.step_freq = get_double( v, "RF step frequency" );
		hp8648b.step_freq_is_set = SET;

		too_many_arguments( v );
	}
	else if ( ! hp8648b.step_freq_is_set )
	{
...
Var *synthesizer_sweep_up( Var *v )
{
	...
	if ( FSC2_MODE == TEST )
		hp8648b.real_attenuation = hp8648b_get_att( hp8648b.freq );
	else
	{
		att = hp8648b_get_att( hp8648b.freq );
		if ( att != hp8648b.real_attenuation )
		{
			hp8648b.real_attenuation = att;
			hp8648b_set_attenuation( att );
...
Var *synthesizer_reset_frequency( Var *v )
{
	...
	if ( FSC2_MODE == TEST )
		hp8648b.freq = hp8648b.start_freq;
	else
		hp8648b.freq = hp8648b_set_frequency( hp8648b.start_freq );
...
Var *synthesizer_use_table( Var *v )
{
	...
	if ( v == NULL )
	{
		if ( DEFAULT_TABLE_FILE[ 0 ] ==  '/' )
			hp8648b.table_file = T_strdup( DEFAULT_TABLE_FILE );
		else
			hp8648b.table_file = get_string( "%s%s%s", libdir, slash( libdir ),
											 DEFAULT_TABLE_FILE );

		if ( ( tfp = hp8648b_open_table( hp8648b.table_file ) ) == NULL )
		{
	...
	else
	{
		...
		TRY
		{
			tfp = hp8648b_find_table( &tfname );
			hp8648b.table_file = tfname;
			TRY_SUCCESS;
		}
		CATCH( EXCEPTION )
		{
...
Var *synthesizer_att_ref_freq( Var *v )
{
	...
	if ( v == NULL )
		return vars_push( FLOAT_VAR, hp8648b.att_ref_freq );
	...
	if ( hp8648b.use_table )
	{
		...
		hp8648b.att_at_ref_freq = hp8648b_get_att_from_table( freq );
	}
...
Var *synthesizer_modulation( Var *v )
{
	...
	while ( v )
	{
		if ( ( 1 << ( ( res = hp8648b_set_mod_param( v, &ampl, &what ) )
					  - 1 ) ) & set )
...
Var *synthesizer_mod_type( Var *v )
{
	...
	if ( v == NULL )
	{
		switch ( FSC2_MODE )
		{
			...
			case TEST :
				if ( ! hp8648b.mod_type_is_set )
				{
					hp8648b.mod_type = MOD_TYPE_FM;
					hp8648b.mod_type_is_set = SET;
				}
				break;

			case EXPERIMENT :
				hp8648b.mod_type = hp8648b_get_mod_type( );
		}

		return vars_push( INT_VAR, hp8648b.mod_type != UNDEFINED ?
						  							  hp8648b.mod_type : - 1 );
	}
	...
	hp8648b.mod_type = res;
	hp8648b.mod_type_is_set = SET;

	if ( FSC2_MODE == EXPERIMENT )
		hp8648b_set_mod_type( res );
...
Var *synthesizer_mod_source( Var *v )
{
	...
	if ( v == NULL )
	{
		...
		if ( hp8648b.mod_type == MOD_TYPE_OFF )
		{
		...
		{
			case PREPARATION :
				if ( ! hp8648b.mod_source_is_set[ hp8648b.mod_type ] )
				{
					print( FATAL, "Modulation source for %s modulation "
						   "hasn't been set yet.\n",
						   mod_types[ hp8648b.mod_type ] );
					THROW( EXCEPTION );
				}
				break;
			...
			case TEST :
				if ( ! hp8648b.mod_source_is_set[ hp8648b.mod_type ] )
				{
					hp8648b.mod_source[ hp8648b.mod_type ] = MOD_SOURCE_AC;
					hp8648b.mod_source_is_set[ hp8648b.mod_type ] = SET;
				}
				break;

			case EXPERIMENT :
				hp8648b.mod_source[ hp8648b.mod_type ] =
			                        hp8648b_get_mod_source( hp8648b.mod_type );
				break;
		}

		return vars_push( INT_VAR, hp8648b.mod_source[ hp8648b.mod_type ] );
	}
	...
	if ( FSC2_MODE != EXPERIMENT )
		hp8648b.mod_source[ hp8648b.mod_type ] = source;
	else
		hp8648b.mod_source[ hp8648b.mod_type ] =
							hp8648b_set_mod_source( hp8648b.mod_type, source );
	hp8648b.mod_source_is_set[ hp8648b.mod_type ] = SET;

	return vars_push( INT_VAR, hp8648b.mod_source );
}
...
Var *synthesizer_mod_ampl( Var *v )
{
	...
	if ( v == NULL )
	{
		...
		{
			case PREPARATION :
				if ( ! hp8648b.mod_ampl_is_set[ hp8648b.mod_type ] )
				{
			...
			case TEST :
				if ( ! hp8648b.mod_ampl_is_set[ hp8648b.mod_type ] )
				{
					hp8648b.mod_ampl[ hp8648b.mod_type ] =
												  defaults[ hp8648b.mod_type ];
					hp8648b.mod_ampl_is_set[ hp8648b.mod_type ] = SET;
				}
				break;

			case EXPERIMENT :
				hp8648b.mod_ampl[ hp8648b.mod_type ] =
			                          hp8648b_get_mod_ampl( hp8648b.mod_type );
				break;
		}

		return vars_push( FLOAT_VAR, hp8648b.mod_ampl[ hp8648b.mod_type ] );
	}
	...
	if ( ampl < 0.0 )
	{
		print( FATAL, "Invalid negative %s modulation amplitude of %g %s.\n",
			   mod_types[ hp8648b.mod_type ],
			   hp8648b.mod_type == MOD_TYPE_FM ? "kHz" :
						  ( hp8648b.mod_type == MOD_TYPE_AM ? "%%" : "rad" ) );
		THROW( EXCEPTION );
	}
	...
	if ( FSC2_MODE != EXPERIMENT )
		hp8648b.mod_ampl[ hp8648b.mod_type ] = ampl;
	else
		hp8648b.mod_ampl[ hp8648b.mod_type ] =
		                              hp8648b_get_mod_ampl( hp8648b.mod_type );
	hp8648b.mod_ampl_is_set[ hp8648b.mod_type ] = SET;
