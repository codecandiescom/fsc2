...
#define LECROY9420_TEST_TIMEBASE     1.0e-3      /* 1 ms per division*/
...
#define LECROY9420_CH_MAX   LECROY9420_CH2


#define GENERAL_TO_LECROY9420 0
#define LECROY9420_TO_GENERAL 1
...
#define LECROY9420_MIN_TIMEBASE 1.0e-9
#define LECROY9420_MAX_TIMEBASE 5.0e3


/* Maximum and minimum sensitivity [V/div] for CH1 and CH2 */

#define LECROY9420_MAX_SENS     5.0e-3     /* 5 mV */
#define LECROY9420_MIN_SENS     2.5        /* 2.5 V */
...
#define LECROY9420_TRG_MAX_LEVEL_CH_FAC 5.0
...
typedef struct LECROY9420 LECROY9420_T;
...
struct LECROY9420 {
	...
	bool is_displayed[ LECROY9420_MAX_CHANNELS ];
	...
	double sens[ LECROY9420_MAX_CHANNELS ];
	bool is_sens[ LECROY9420_MAX_CHANNELS ];

	double offset[ LECROY9420_MAX_CHANNELS ];
	bool is_offset[ LECROY9420_MAX_CHANNELS ];

	bool bandwidth_limiter;
	bool is_bandwidth_limiter;

	int coupling[ LECROY9420_MAX_CHANNELS ];
	bool is_coupling[ LECROY9420_MAX_CHANNELS ];

	int trigger_channel;
	bool is_trigger_channel;

	double trigger_level[ LECROY9420_MAX_CHANNELS ];
	bool is_trigger_level[ LECROY9420_MAX_CHANNELS ];

	int trigger_slope[ LECROY9420_MAX_CHANNELS ];
	bool is_trigger_slope[ LECROY9420_MAX_CHANNELS ];

	int trigger_coupling[ LECROY9420_MAX_CHANNELS ];
	bool is_trigger_coupling[ LECROY9420_MAX_CHANNELS ];
	...
	bool is_avg_setup[ LECROY9420_MAX_CHANNELS ];
	long source_ch[ LECROY9420_MAX_CHANNELS ];
...
HORI_RES_T hres[ 39 ] = { { 1.0e-9, 1.0e-10,   100,   -1.0,    -1 },
					      ...
					      { 5.0e-7, 1.0e-09, 50000, 1.0e-8,  5000 },
...
extern LECROY9420_T lecroy9420;

extern const char *LECROY9420_Channel_Names[ 13 ];

extern bool lecroy9420_IN_SETUP;
...
int lecroy9420_init_hook( void );
int lecroy9420_test_hook( void );
int lecroy9420_exp_hook( void );
int lecroy9420_end_of_exp_hook( void );
...
bool lecroy9420_init( const char *name );
...
bool lecroy9420_set_sens( int channel, double sens );
double lecroy9420_get_offset( int channel );
bool lecroy9420_set_offset( int channel, double offset );
int lecroy9420_get_coupling( int channel );
bool lecroy9420_set_coupling( int channel, int type );
...
bool lecroy9420_set_trigger_level( int channel, double level );
double lecroy9420_get_trigger_slope( int channel );
bool lecroy9420_set_trigger_slope( int channel, int slope );
int lecroy9420_get_trigger_coupling( int channel );
int lecroy9420_set_trigger_coupling( int channel, int cpl );
...
bool lecroy9420_display( int ch, int on_off );
long lecroy9420_get_num_avg( int channel );
bool lecroy9420_get_desc( int channel );
void lecroy9420_set_up_averaging( long channel, long source, long num_avg,
								  long rec_len );
void lecroy9420_finished( void );
void lecroy9420_start_acquisition( void );
void lecroy9420_get_curve( int ch, Window_T *w, double **array, long *length );
double lecroy9420_get_area( int ch, Window_T *w );
double lecroy9420_get_amplitude( int ch, Window_T *w );
void lecroy9420_free_running( void );
void lecroy9420_copy_curve( long src, long dest );
bool lecroy9420_command( const char *cmd );


const char *lecroy9420_ptime( double p_time );
void lecroy9420_delete_windows( LECROY9420_T *s );
Window_T *lecroy9420_get_window_by_number( long wid );
void lecroy9420_all_windows_check( void );
void lecroy9420_window_check( Window_T *w, bool show_num );
void lecroy9420_length_check( long len );
long lecroy9420_find_length( void );
long lecroy9420_curve_length( void );
double lecroy9420_time_per_point( void );
long lecroy9420_translate_channel( int dir, long channel, bool flag );
void lecroy9420_store_state( LECROY9420_T *dest, LECROY9420_T *src );
