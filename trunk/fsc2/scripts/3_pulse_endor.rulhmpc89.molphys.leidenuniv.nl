#!/usr/bin/perl
# -*- cperl -*-
# Generated by fsc2_guify from 3_pulse_endor_w.EDL on Mon Oct 27 12:56:53 CET 2008
...
$fsc2_apply_button->pack( %wp, -padx => '25', -pady => '10' );
$fsc2_quit_button->pack(  %wp, -padx => '25', -pady => '10' );
...
sub write_out {
    open my $fh, '|-', 'fsc2_' . lc $fsc2_how_to_run
...
sub store_defs {
    ...
    if (     $START_FREQ{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
    ...
    if (     $END_FREQ{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
    ...
    if (     $FREQ_STEP{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
    ...
    if (     $WAIT_TIME{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
