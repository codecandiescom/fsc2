#!/usr/bin/perl
# -*- cperl -*-
# Generated by fsc2_guify from 2_pulse_epr_w.EDL on Sat Mar 28 12:15:19 CET 2009
...
my %PULSE_DIST;
push @VARS, \%PULSE_DIST;
$PULSE_DIST{ is_valid } = 1;
$PULSE_DIST{ type } = 'int';
...
-width => 10,
-validate => 'key',
-validatecommand => sub{ int_check( shift, \$PULSE_DIST{ is_valid },
...
-balloonmsg  => 'Range: [ ' . ( defined $PULSE_DIST{ min } ? $PULSE_DIST{ min } : '-inf' ) . ' : ' . ( defined $PULSE_DIST{ max } ? $PULSE_DIST{ max } : '+inf' ) . ' ]' . ( $PULSE_DIST{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
...
sub write_out {
    ...
    print $fh "/* \$Id: 2_pulse_epr_w.EDL 8121 2008-05-14 20:22:15Z jens \$ */
...
# === if ( ADC_CHANNELS eq "0 and 1" )
    if ( eval { ( $ADC_CHANNELS eq "0 and 1" ) } ) {
        print $fh "init_1d( 4, N_Points, \"Field [G]\", \"Echo amplitude [a.u.]\" );
";
# === else
    } else {
        print $fh "init_1d( 2, N_Points, \"Field [G]\", \"Echo amplitude [a.u.]\" );
";
# === endif
    }
...
# === if ( START_FIELD <= END_FIELD )
    if ( eval { ( $START_FIELD <= $END_FIELD ) } ) {
        print $fh "change_scale_1d( start_field, field_step );
";
# === else
    } else {
        print $fh "change_scale_1d( end_field, - field_step );
";
# === endif
    }
...
# === if ( START_FIELD <= END_FIELD )
    if ( eval { ( $START_FIELD <= $END_FIELD ) } ) {
        print $fh "        display_1d( I, data[ 1, J, I ], 1,
                    I, data[ 2, J, I ], 2,
                    I, avg[ 1, I ],     3,
                    I, avg[ 2, I ],     4 );
";
# === else
        ...
        print $fh "        display_1d( N_Points - I + 1, data[ 1, J, I ], 1,
...
# === if ( ADC_CHANNELS eq "0 and 1" )
    if ( eval { ( $ADC_CHANNELS eq "0 and 1" ) } ) {
        print $fh "            avg[ 1 ] = float_slice( N_Points );
...
# === else
        ...
        print $fh "            avg = float_slice( N_Points );
            FOR K = 1 : J {
...
sub store_defs {
    ...
    if (    (     $PULSE_DIST{ type } =~ /^int_empty$/o
              and $PULSE_DIST{ value } eq '' )
         or (     $PULSE_DIST{ value } ne ''
              and $PULSE_DIST{ value } =~ /^[+-]?\d+$/o
              and ( defined $PULSE_DIST{ max } ? $PULSE_DIST{ max } >= $PULSE_DIST{ value } : 1 )
              and ( defined $PULSE_DIST{ min } ? $PULSE_DIST{ min } <= $PULSE_DIST{ value } : 1 ) ) ) {
...
sub load_defs {
    ...
    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?\d+$/;
    chomp $ne;
    goto done_reading if    ( defined $PULSE_DIST{ max } and $ne > $PULSE_DIST{ max } )
                         or ( defined $PULSE_DIST{ min } and $ne < $PULSE_DIST{ min } );
    $PULSE_DIST{ value } = $ne;
