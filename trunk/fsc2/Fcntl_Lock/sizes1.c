/*
  ...
  Copyright (C) 2002-2006 Jens Thoms Toerring <jt@toering.de>
...
int main( void )
{
	...
	else if ( sizeof( f.l_type ) == sizeof( long long ) )
		fprintf( fp, "#define LTYPE_TYPE      long long\n" );
	else
	{
		fclose( fp );
		unlink( "Fcntl_Lock.h" );
	...
	else if ( sizeof( f.l_whence ) == sizeof( long long ) )
		fprintf( fp, "#define LWHENCE_TYPE    long long\n" );
	else
	{
		fclose( fp );
		unlink( "Fcntl_Lock.h" );
	...
	else if ( sizeof( f.l_start ) == sizeof( long long ) )
		fprintf( fp, "#define LSTART_TYPE     long long\n" );
	else
	{
		fclose( fp );
		unlink( "Fcntl_Lock.h" );
	...
	else if ( sizeof( f.l_len ) == sizeof( long long ) )
		fprintf( fp, "#define LLEN_TYPE       long long\n" );
	else
	{
		fclose( fp );
		unlink( "Fcntl_Lock.h" );
	...
	else if ( sizeof( f.l_pid ) == sizeof( long long ) )
		fprintf( fp, "#define LPID_TYPE       long long\n" );
	else
	{
		fclose( fp );
		unlink( "Fcntl_Lock.h" );
