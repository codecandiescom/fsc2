#!/usr/bin/perl
# -*- cperl -*-
# Generated by fsc2_guify from 3_pulse_T1_w.EDL on Sat Mar 28 12:15:20 CET 2009
...
my %P1_P2_INCR;
push @VARS, \%P1_P2_INCR;
$P1_P2_INCR{ is_valid } = 1;
$P1_P2_INCR{ type } = 'int';
...
-width => 10,
-validate => 'key',
-validatecommand => sub{ int_check( shift, \$P1_P2_INCR{ is_valid },
...
-balloonmsg  => 'Range: [ ' . ( defined $P1_P2_INCR{ min } ? $P1_P2_INCR{ min } : '-inf' ) . ' : ' . ( defined $P1_P2_INCR{ max } ? $P1_P2_INCR{ max } : '+inf' ) . ' ]' . ( $P1_P2_INCR{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
...
sub write_out {
    ...
    print $fh "/* \$Id: 3_pulse_T1_w.EDL 8121 2008-05-14 20:22:15Z jens \$ */
...
fsave( File,
	   \"\\n\"
       \"% Date:                   # #\\n\"
       \"% Script:                 3_pulse_T1 (W-band, Leiden)\\n\"
       \"% Field:                  # G\\n\"
       \"% Repetition time:        # ms\\n\"
...
sub store_defs {
    ...
    if (    (     $P1_P2_INCR{ type } =~ /^int_empty$/o
              and $P1_P2_INCR{ value } eq '' )
         or (     $P1_P2_INCR{ value } ne ''
              and $P1_P2_INCR{ value } =~ /^[+-]?\d+$/o
              and ( defined $P1_P2_INCR{ max } ? $P1_P2_INCR{ max } >= $P1_P2_INCR{ value } : 1 )
              and ( defined $P1_P2_INCR{ min } ? $P1_P2_INCR{ min } <= $P1_P2_INCR{ value } : 1 ) ) ) {
...
sub load_defs {
    ...
    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?\d+$/;
    chomp $ne;
    goto done_reading if    ( defined $P1_P2_INCR{ max } and $ne > $P1_P2_INCR{ max } )
                         or ( defined $P1_P2_INCR{ min } and $ne < $P1_P2_INCR{ min } );
    $P1_P2_INCR{ value } = $ne;
