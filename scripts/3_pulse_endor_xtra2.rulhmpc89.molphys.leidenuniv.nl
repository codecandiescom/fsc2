#!/usr/bin/perl
# -*- cperl -*-
# Generated by fsc2_guify from 3_pulse_endor_xtra2_w.EDL on Sat Mar 28 12:15:20 CET 2009
...
my %FREQ_STEP_DELAY;
push @VARS, \%FREQ_STEP_DELAY;
$FREQ_STEP_DELAY{ is_valid } = 1;
$FREQ_STEP_DELAY{ type } = 'int';
$FREQ_STEP_DELAY{ tk_frame } = $fsc2_main_frame->Frame( );
$FREQ_STEP_DELAY{ tk_label } = $FREQ_STEP_DELAY{ tk_frame }->Label( -text => "Frequency step delay",
-width => 20,
-anchor => 'w' );
$FREQ_STEP_DELAY{ value } = 20;

$FREQ_STEP_DELAY{ min } = 0;
$FREQ_STEP_DELAY{ max } = 1000;
$FREQ_STEP_DELAY{ tk_entry } = $FREQ_STEP_DELAY{ tk_frame }->Entry( -textvariable => \$FREQ_STEP_DELAY{ value },
-width => 10,
-validate => 'key',
-validatecommand => sub{ int_check( shift, \$FREQ_STEP_DELAY{ is_valid },
( defined $FREQ_STEP_DELAY{ min } ? $FREQ_STEP_DELAY{ min } : undef ),
( defined $FREQ_STEP_DELAY{ max } ? $FREQ_STEP_DELAY{ max } : undef ) ); },
-relief => 'sunken' );
$fsc2_balloon->attach( $FREQ_STEP_DELAY{ tk_entry },
-balloonmsg  => 'Range: [ ' . ( defined $FREQ_STEP_DELAY{ min } ? $FREQ_STEP_DELAY{ min } : '-inf' ) . ' : ' . ( defined $FREQ_STEP_DELAY{ max } ? $FREQ_STEP_DELAY{ max } : '+inf' ) . ' ]' . ( $FREQ_STEP_DELAY{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
$FREQ_STEP_DELAY{ tk_unit } = $FREQ_STEP_DELAY{ tk_frame }->Label( -text => "ms",
-width => 5 );
$FREQ_STEP_DELAY{ tk_frame }->pack( %fp );
$FREQ_STEP_DELAY{ tk_label }->pack( %wp );
$FREQ_STEP_DELAY{ tk_entry }->pack( %wp );
$FREQ_STEP_DELAY{ tk_unit  }->pack( %up );
...
sub write_out {
    ...
    my $FREQ_STEP_DELAY = $FREQ_STEP_DELAY{ value };
...
# === endif
    ...
    print $fh "                }
	        ...
	        fsave( File2,
				   \"\\n\"
	               \"% Date:                   # #\\n\"
	               \"% Script:                 3_pulse_endor_xtra2 (W-band, Leiden)\\n\"
	               \"% Field:                  # G\\n\"
	               \"% Start frequency:        # MHz\\n\"
	               \"% End frequency:          # MHz\\n\"
	               \"% Frequency step:         # MHz\\n\"
...
# === endif
    ...
    fsave( File1,
		   \"\\n\"
           \"% Date:                   # #\\n\"
           \"% Script:                 3_pulse_endor_xstra2 (W-band, Leiden)\\n\"
           \"% Field:                  # G\\n\"
           \"% Start frequency:        # MHz\\n\"
           \"% End frequency:          # MHz\\n\"
           \"% Frequency step:         # MHz\\n\"
...
sub store_defs {
    ...
    if (    (     $FREQ_STEP_DELAY{ type } =~ /^int_empty$/o
              and $FREQ_STEP_DELAY{ value } eq '' )
         or (     $FREQ_STEP_DELAY{ value } ne ''
              and $FREQ_STEP_DELAY{ value } =~ /^[+-]?\d+$/o
              and ( defined $FREQ_STEP_DELAY{ max } ? $FREQ_STEP_DELAY{ max } >= $FREQ_STEP_DELAY{ value } : 1 )
              and ( defined $FREQ_STEP_DELAY{ min } ? $FREQ_STEP_DELAY{ min } <= $FREQ_STEP_DELAY{ value } : 1 ) ) ) {
        print $fh "$FREQ_STEP_DELAY{ value }\n";
    } else {
        print $fh "20\n";
    }
...
sub load_defs {
    ...
    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?\d+$/;
    chomp $ne;
    goto done_reading if    ( defined $FREQ_STEP_DELAY{ max } and $ne > $FREQ_STEP_DELAY{ max } )
                         or ( defined $FREQ_STEP_DELAY{ min } and $ne < $FREQ_STEP_DELAY{ min } );
    $FREQ_STEP_DELAY{ value } = $ne;
