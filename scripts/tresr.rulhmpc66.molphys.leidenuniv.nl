#!/usr/bin/perl
# -*- cperl -*-
# Generated by fsc2_guify from tresr_j.EDL on Mon Mar 23 18:50:31 CET 2009
...
# === SIGNAL_FIELD float [ 0 : 120019 ] [ 80050 ]  "Signal start field\n(for laser correction)" "G"

my %SIGNAL_FIELD;
push @VARS, \%SIGNAL_FIELD;
$SIGNAL_FIELD{ is_valid } = 1;
$SIGNAL_FIELD{ type } = 'float';
...
$SIGNAL_FIELD{ min } = 0;
$SIGNAL_FIELD{ max } = 120019;
$SIGNAL_FIELD{ tk_entry } = $SIGNAL_FIELD{ tk_frame }->Entry( -textvariable => \$SIGNAL_FIELD{ value },
-width => 10,
-validate => 'key',
-validatecommand => sub{ float_check( shift, \$SIGNAL_FIELD{ is_valid },
...
-balloonmsg  => 'Range: [ ' . ( defined $SIGNAL_FIELD{ min } ? $SIGNAL_FIELD{ min } : '-inf' ) . ' : ' . ( defined $SIGNAL_FIELD{ max } ? $SIGNAL_FIELD{ max } : '+inf' ) . ' ]' . ( $SIGNAL_FIELD{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
...
my %PRETRIGGER;
push @VARS, \%PRETRIGGER;
$PRETRIGGER{ is_valid } = 1;
$PRETRIGGER{ type } = 'float';
...
-width => 10,
-validate => 'key',
-validatecommand => sub{ float_check( shift, \$PRETRIGGER{ is_valid },
...
-balloonmsg  => 'Range: [ ' . ( defined $PRETRIGGER{ min } ? $PRETRIGGER{ min } : '-inf' ) . ' : ' . ( defined $PRETRIGGER{ max } ? $PRETRIGGER{ max } : '+inf' ) . ' ]' . ( $PRETRIGGER{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
...
-width => 10,
-validate => 'key',
-validatecommand => sub{ int_check( shift, \$NUM_AVG{ is_valid },
( defined $NUM_AVG{ min } ? $NUM_AVG{ min } : undef ),
( defined $NUM_AVG{ max } ? $NUM_AVG{ max } : undef ) ); },
-relief => 'sunken' );
$fsc2_balloon->attach( $NUM_AVG{ tk_entry },
-balloonmsg  => 'Range: [ ' . ( defined $NUM_AVG{ min } ? $NUM_AVG{ min } : '-inf' ) . ' : ' . ( defined $NUM_AVG{ max } ? $NUM_AVG{ max } : '+inf' ) . ' ]' . ( $NUM_AVG{ type } =~ /_empty$/ ? ' or leave empty' : '' ) );
...
sub store_defs {
    ...
    if (    (     $SIGNAL_FIELD{ type } =~ /^float_empty$/o
              and $SIGNAL_FIELD{ value } eq '' )
         or (     $SIGNAL_FIELD{ value } ne ''
              and $SIGNAL_FIELD{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
              and ( defined $SIGNAL_FIELD{ max } ? $SIGNAL_FIELD{ max } >= $SIGNAL_FIELD{ value } : 1 )
              and ( defined $SIGNAL_FIELD{ min } ? $SIGNAL_FIELD{ min } <= $SIGNAL_FIELD{ value } : 1 ) ) ) {
        print $fh "$SIGNAL_FIELD{ value }\n";
    } else {
        print $fh "80050\n";
    }

    if (    (     $PRETRIGGER{ type } =~ /^float_empty$/o
              and $PRETRIGGER{ value } eq '' )
         or (     $PRETRIGGER{ value } ne ''
              and $PRETRIGGER{ value } =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o
              and ( defined $PRETRIGGER{ max } ? $PRETRIGGER{ max } >= $PRETRIGGER{ value } : 1 )
              and ( defined $PRETRIGGER{ min } ? $PRETRIGGER{ min } <= $PRETRIGGER{ value } : 1 ) ) ) {
        print $fh "$PRETRIGGER{ value }\n";
    } else {
        print $fh "10.0\n";
    }

    if (    (     $NUM_AVG{ type } =~ /^int_empty$/o
              and $NUM_AVG{ value } eq '' )
         or (     $NUM_AVG{ value } ne ''
              and $NUM_AVG{ value } =~ /^[+-]?\d+$/o
              and ( defined $NUM_AVG{ max } ? $NUM_AVG{ max } >= $NUM_AVG{ value } : 1 )
              and ( defined $NUM_AVG{ min } ? $NUM_AVG{ min } <= $NUM_AVG{ value } : 1 ) ) ) {
...
sub load_defs {
    ...
    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o;
    chomp $ne;
    goto done_reading if    ( defined $SIGNAL_FIELD{ max } and $ne > $SIGNAL_FIELD{ max } )
                         or ( defined $SIGNAL_FIELD{ min } and $ne < $SIGNAL_FIELD{ min } );
    $SIGNAL_FIELD{ value } = $ne;

    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?((\d+(\.(\d+)?)?)|(\.\d+))([eE][+-]?\d+)?$/o;
    chomp $ne;
    goto done_reading if    ( defined $PRETRIGGER{ max } and $ne > $PRETRIGGER{ max } )
                         or ( defined $PRETRIGGER{ min } and $ne < $PRETRIGGER{ min } );
    $PRETRIGGER{ value } = $ne;

    goto done_reading unless     defined( $ne = <$fh> )
                             and $ne =~ /^[+-]?\d+$/;
    chomp $ne;
    goto done_reading if    ( defined $NUM_AVG{ max } and $ne > $NUM_AVG{ max } )
                         or ( defined $NUM_AVG{ min } and $ne < $NUM_AVG{ min } );
    $NUM_AVG{ value } = $ne;
