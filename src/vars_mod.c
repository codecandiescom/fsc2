...
static Var_T * vars_mod_i( Var_T * v1,
                           Var_T * v2,
                           bool    exc );
static Var_T * vars_int_var_mod( Var_T * v1,
                                 Var_T * v2,
                                 bool    exc );
static Var_T * vars_float_var_mod( Var_T * v1,
                                   Var_T * v2,
                                   bool    exc );
static Var_T * vars_int_arr_mod( Var_T * v1,
                                 Var_T * v2,
                                 bool    exc );
static Var_T * vars_float_arr_mod( Var_T * v1,
                                   Var_T * v2,
                                   bool    exc );
static Var_T * vars_ref_mod( Var_T * v1,
                             Var_T * v2,
                             bool    exc );
                  ...
                  bool    exc )
{
    ...
    switch ( v2->type )
    {
        case INT_VAR :
            if ( ! exc )
            {
                vars_mod_check( ( double ) v2->val.lval );
                ir = v1->val.lval % v2->val.lval;
            }
            else
            {
                vars_mod_check( ( double ) v1->val.lval );
        ...
        case FLOAT_VAR :
            if ( ! exc )
            {
                vars_mod_check( v2->val.dval );
                dr = fmod( ( double ) v1->val.lval, v2->val.dval );
            }
            else
            {
                vars_mod_check( ( double ) v1->val.lval );
                dr = fmod( v2->val.dval, ( double ) v1->val.lval );
            }
        ...
        case FLOAT_ARR :
            ...
            for ( i = 0; i < v2->len; i++ )
            {
                if ( ! exc )
                {
                    ...
                    dr = fmod( ( double ) v1->val.lval,
                               new_var->val.dpnt[ i ] );
                }
                else
                {
                    vars_mod_check( ( double ) v1->val.lval );
                    dr = fmod( new_var->val.dpnt[ i ],
                               ( double ) v1->val.lval );
                }
        ...
        case FLOAT_REF :
            ...
            while ( ( gp = vars_iter( new_var ) ) != NULL )
            {
                if ( ! exc )
                {
                    vars_mod_check( * ( double * ) gp );
                    dr = fmod( ( double ) v1->val.lval, * ( double * ) gp );
                }
                else
                {
                    vars_mod_check( ( double ) v1->val.lval );
                    dr = fmod( * ( double * ) gp, ( double ) v1->val.lval );
                }
                * ( double * ) gp = dr;
            }
                    ...
                    bool    exc )
{
    ...
    switch ( v2->type )
    {
        ...
        case FLOAT_VAR :
            ...
            {
                vars_mod_check( v2->val.dval );
                dr = fmod( v1->val.dval, v2->val.dval );
            }
            else
            {
                vars_mod_check( v1->val.dval );
                dr = fmod( v2->val.dval, v1->val.dval );
            }
            new_var = vars_push( FLOAT_VAR, dr );
            vars_pop( v1 );
            vars_pop( v2 );
        ...
        case INT_ARR :
            ...
            for ( i = 0; i < new_var->len; i++ )
            {
                if ( ! exc )
                {
                    vars_mod_check( ( double ) v2->val.lpnt[ i ] );
                    dr = fmod( v1->val.dval, ( double ) v2->val.lpnt[ i ] );
                }
                else
                {
                    vars_mod_check( ( double ) v2->val.lpnt[ i ] );
                    dr = fmod( ( double ) v2->val.lpnt[ i ], v1->val.dval );
                }
                new_var->val.dpnt[ i ] = dr;
            }
        ...
        case FLOAT_ARR :
            ...
            for ( i = 0; i < new_var->len; i++ )
            {
                ...
                {
                    vars_mod_check( new_var->val.dpnt[ i ] );
                    dr = fmod( v1->val.dval, new_var->val.dpnt[ i ] );
                }
                else
                {
                    vars_mod_check( v1->val.dval );
                    dr = fmod( new_var->val.dpnt[ i ], v1->val.dval );
                }
                new_var->val.dpnt[ i ] = dr;
            }
        ...
        case INT_REF :
            ...
            while ( ( gp = vars_iter( new_var ) ) != NULL )
            {
                ...
                {
                    vars_mod_check( * ( double * ) gp );
                    dr = fmod( v1->val.dval, * ( double * ) gp );
                }
                else
                {
                    vars_mod_check( v1->val.dval );
                    dr = fmod( * ( double * ) gp, v1->val.dval );
                }
                * ( double * ) gp = dr;
            }
    ...
    switch ( v2->type )
    {
        ...
        case INT_ARR :
            ...
            for ( i = 0; i < new_var->len; i++ )
            {
                ...
                {
                    vars_mod_check( ( double ) v1->val.lpnt[ i ] );
        ...
        case INT_REF : case FLOAT_REF :
            ...
            for ( i = 0; i < new_var->len; i++ )
                vars_mod_i( vars_push( INT_ARR, v1->val.lpnt, v1->len ),
    ...
    switch ( v2->type )
    {
        ...
        case FLOAT_ARR :
            ...
            for ( i = 0; i < new_var->len; i++ )
            {
                ...
                {
                    vars_mod_check( new_var->val.dpnt[ i ] );
                    dr = fmod( v1->val.dpnt[ i ], new_var->val.dpnt[ i ] );
                }
                else
                {
                    vars_mod_check( v1->val.dpnt[ i ] );
                    dr = fmod( new_var->val.dpnt[ i ], v1->val.dpnt[ i ] );
                }
                new_var->val.dpnt[ i ] = dr;
            }
        ...
        case INT_REF : case FLOAT_REF :
            ...
            for ( i = 0; i < new_var->len; i++ )
                vars_mod_i( vars_push( FLOAT_ARR, v1->val.dpnt, v1->len ),
                            new_var->val.vptr[ i ], exc );
