...
ag54830b_l_init_hook( void )
{
	...
	ag54830b_l.is_trig_pos       = UNSET;
	ag54830b_l.data_source       = AG54830B_L_UNDEF;
...
int
ag54830b_l_exp_hook( void )
{
	/* Initialize the digitizer*/

	if ( ! ag54830b_l_init( DEVICE_NAME ) )
	{
...
digitizer_show_channel( Var_T * v )
{
	...
	channel = ag54830b_l_translate_channel( GENERAL_TO_AG54830B_L,
							   get_strict_long( v, "channel number" ), UNSET );

	if ( channel >= MAX_CHANNELS )
	{
		print( FATAL, "Invalid channel number: %ld.\n",
			   ag54830b_l_translate_channel( AG54830B_L_TO_GENERAL,
										   channel, UNSET ) );
		THROW( EXCEPTION );
	}
	...
	if ( ( v = vars_pop( v ) ) == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.channel_is_on[ channel ] )
					no_query_possible( );
				return vars_push( INT_VAR,
								  ag54830b_l.channel_is_on[ channel ] );

			case TEST :
				return vars_push( INT_VAR,
								  ag54830b_l.channels_in_use[ channel ] ?
								  ag54830b_l.channel_is_on[ channel ] :
								  AG54830B_L_TEST_CHANNEL_STATE );

			case EXPERIMENT :
				ag54830b_l.channel_is_on[ channel ] =
								   ag54830b_l_display_channel_state( channel );
				ag54830b_l.channels_in_use[ channel ] = SET;
				return vars_push( INT_VAR,
								  ag54830b_l.channel_is_on[ channel ] );
		}
	...
	if (    FSC2_MODE == EXPERIMENT
		 && ag54830b_l_display_channel_state( channel ) != state )
	{
		sprintf( cmd, ":%s:DISP %i\n",
				 AG54830B_L_Channel_Names[ channel ], state );
		ag54830b_l_command( cmd );
	};

	ag54830b_l.channel_is_on[ channel ] = state;
	ag54830b_l.channels_in_use[ channel ] = SET;

	return vars_push( INT_VAR, ag54830b_l.channel_is_on[ channel ] );
}
...
digitizer_timebase( Var_T * v )
{
	...
	if ( v == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.is_timebase )
					no_query_possible( );
				return vars_push( FLOAT_VAR, ag54830b_l.timebase );

			case TEST :
				return vars_push( FLOAT_VAR, ag54830b_l.is_timebase ?
								 ag54830b_l.timebase :
								  AG54830B_L_TEST_TIME_BASE );
	...
	ag54830b_l.is_timebase = SET;

	if ( FSC2_MODE == EXPERIMENT )
		ag54830b_l_set_timebase( timebase );

	ag54830b_l.timebase = timebase;
...
digitizer_time_per_point( Var_T * v  UNUSED_ARG )
{
	...
	{
		case PREPARATION :
			if ( ! ag54830b_l.is_time_per_point )
				no_query_possible( );
			return vars_push( FLOAT_VAR, ag54830b_l.time_per_point );

		case TEST :
			return vars_push( FLOAT_VAR, ag54830b_l.is_time_per_point ?
							  ag54830b_l.timebase :
							  AG54830B_L_TEST_TIME_PER_POINT );
...
digitizer_sensitivity( Var_T * v )
{
	...
	if ( ( v = vars_pop( v ) ) == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.is_sens[ channel ] )
					no_query_possible( );
				return vars_push( FLOAT_VAR, ag54830b_l.sens[ channel ] );

			case TEST :
				return vars_push( FLOAT_VAR, ag54830b_l.is_sens[ channel ] ?
								  ag54830b_l.sens[ channel ] :
								  AG54830B_L_TEST_SENSITIVITY );

			case EXPERIMENT :
				ag54830b_l.sens[ channel ] = ag54830b_l_get_sens( channel );
				ag54830b_l.is_sens[ channel ] = SET;
				return vars_push( FLOAT_VAR, ag54830b_l.sens[ channel ] );
		}
	...
	if ( FSC2_MODE == EXPERIMENT )
		ag54830b_l_set_sens( channel, sens );

	ag54830b_l.sens[ channel ] = sens;
	ag54830b_l.is_sens[ channel ] = SET;
...
digitizer_num_averages( Var_T * v )
{
	...
	if ( v == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.is_num_avg )
					no_query_possible( );
				return vars_push( INT_VAR, ag54830b_l.num_avg );

			case TEST :
				return vars_push( INT_VAR, ag54830b_l.is_num_avg ?
								  ag54830b_l.num_avg :
								  AG54830B_L_TEST_NUM_AVG );

			case EXPERIMENT :
				ag54830b_l.num_avg = ag54830b_l_get_num_avg( );
				return vars_push( INT_VAR, ag54830b_l.num_avg );
		}
	...
	if ( FSC2_MODE == EXPERIMENT )
		ag54830b_l_set_num_avg( num_avg );

	ag54830b_l.num_avg = num_avg;
	ag54830b_l.is_num_avg = SET;
...
digitizer_record_length( Var_T * v )
{
	...
	if ( v == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.is_rec_len )
					no_query_possible( );
				return vars_push( INT_VAR, ag54830b_l.rec_len );

			case TEST :
				return vars_push( INT_VAR, ag54830b_l.is_rec_len ?
								  ag54830b_l.rec_len :
								  AG54830B_L_TEST_REC_LEN );
	...
	ag54830b_l.is_rec_len = SET;

	too_many_arguments( v );

	if ( FSC2_MODE == EXPERIMENT )
		ag54830b_l_set_record_length( rec_len );
	else
		ag54830b_l.rec_len = rec_len;
...
digitizer_trigger_position( Var_T * v )
{
	...
	if ( v == NULL )
		...
		{
			case PREPARATION :
				if ( ! ag54830b_l.is_trig_pos )
					no_query_possible( );
				return vars_push( FLOAT_VAR, ag54830b_l.trig_pos );

			case TEST :
				return vars_push( FLOAT_VAR, ag54830b_l.is_trig_pos ?
								  ag54830b_l.trig_pos :
								  AG54830B_L_TEST_TRIG_POS );

			case EXPERIMENT :
				ag54830b_l.trig_pos = ag54830b_l_get_trigger_pos( );
				return vars_push( FLOAT_VAR, ag54830b_l.trig_pos );
		}
	...
	ag54830b_l.is_trig_pos = SET;

	if ( FSC2_MODE == EXPERIMENT )
		ag54830b_l_set_trigger_pos( trig_pos );

	ag54830b_l.trig_pos = trig_pos;
...
digitizer_start_acquisition( Var_T * v  UNUSED_ARG )
{
	if ( FSC2_MODE == EXPERIMENT )
	{
		ag54830b_l_command_retry( "*CLS;:DIGITIZE;*OPC\n");
		ag54830b_l.acquisition_is_running = SET;
		ag54830b_l_acq_completed();
	}
...
digitizer_get_curve( Var_T * v )
{
	...
	ch = ( int ) ag54830b_l_translate_channel( GENERAL_TO_AG54830B_L,
							   get_strict_long( v, "channel number" ), UNSET );

	if ( ch >= NUM_DISPLAYABLE_CHANNELS )
	{
		print( FATAL, "Invalid channel %s.\n", AG54830B_L_Channel_Names[ ch ] );
		THROW( EXCEPTION );
	}

	ag54830b_l.channels_in_use[ ch ] = SET;
	...
	if ( FSC2_MODE == EXPERIMENT )
	{
		ag54830b_l_get_curve( ch,  &array, &length, SET );
		nv = vars_push( FLOAT_ARR, array, length );
	}
	else
	{
		if ( ag54830b_l.is_rec_len  )
			length = ag54830b_l.rec_len;
		else
			length = AG54830B_L_TEST_REC_LEN;
...
digitizer_get_curve_fast( Var_T * v )
{
	...
	if ( FSC2_MODE == EXPERIMENT )
	{
		ag54830b_l_get_curve( ch,  &array, &length, UNSET );
		nv = vars_push( FLOAT_ARR, array, length );
	}
	else
	{
...
digitizer_get_xorigin( Var_T * v )
{
	...
	if ( FSC2_MODE == EXPERIMENT )
		xorg = ag54830b_l_get_xorigin( channel );
...
digitizer_get_xincrement( Var_T * v )
{
	...
	if ( FSC2_MODE == EXPERIMENT )
		xinc = ag54830b_l_get_xincrement(channel);
...
digitizer_get_yorigin( Var_T * v )
{
	...
	if ( FSC2_MODE == EXPERIMENT )
		yorg = ag54830b_l_get_yorigin(channel);
...
digitizer_get_yincrement( Var_T * v )
{
	...
	if ( FSC2_MODE == EXPERIMENT )
		yinc = ag54830b_l_get_yincrement(channel);

	return vars_push( FLOAT_VAR, yinc );
}

/*------------------------------------------------------*
 * Function for sending a command directly to digitizer
 *------------------------------------------------------*/
