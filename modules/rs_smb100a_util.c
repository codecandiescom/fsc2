...
FILE *
rs_smb100a_find_table( char ** name )
{
    FILE *tfp;
    char *new_name;
    ...
    if ( ( tfp = rs_smb100a_open_table( *name ) ) != NULL )
    {
        rs_smb100a.table_file = T_strdup( *name );
        return tfp;
    }
...
FILE *
rs_smb100a_open_table( char * name )
{
    FILE *tfp;
...
double
rs_smb100a_get_att_from_table( double freq )
{
    ...
    if ( freq < rs_smb100a.min_table_freq )
    {
        print( WARN, "Frequency of %f MHz not covered by table, "
               "interpolating.\n", freq * 1.0e-6 );
        return rs_smb100a.att_table[ 0 ].att;
    }

    if ( freq > rs_smb100a.max_table_freq )
    {
        print( WARN, "Frequency of %f MHz not covered by table, "
               "interpolating.\n", freq * 1.0e-6 );
        return rs_smb100a.att_table[ rs_smb100a.att_table_len - 1 ].att;
    }
    ...
    i_low = 0;
    i_high = rs_smb100a.att_table_len - 1;

    if ( freq == rs_smb100a.att_table[ i_low ].freq )
        return rs_smb100a.att_table[ i_low ].att;
    if ( freq == rs_smb100a.att_table[ i_high ].freq )
        return rs_smb100a.att_table[ i_high ].att;

    i_cur = lrnd( floor( ( i_high - i_low ) *
                         ( freq - rs_smb100a.att_table[ i_low ].freq ) /
                         ( rs_smb100a.att_table[ i_high ].freq
                           - rs_smb100a.att_table[ i_low ].freq ) ) ) + i_low;

    if ( freq > rs_smb100a.att_table[ i_cur ].freq )
    {
        i_low = i_cur;
        if ( freq < rs_smb100a.att_table[ i_cur + 1 ].freq )
            i_high = i_cur + 1;
    }
    else
        i_high = i_cur;
    ...
    att =   ( rs_smb100a.att_table[ i_high ].att
              - rs_smb100a.att_table[ i_low ].att )
          * ( freq - rs_smb100a.att_table[ i_low ].freq )
          / (   rs_smb100a.att_table[ i_high ].freq
              - rs_smb100a.att_table[ i_low ].freq )
          + rs_smb100a.att_table[ i_low ].att;
...
double
rs_smb100a_get_att( double freq )
{
    double att;

    if ( ! rs_smb100a.use_table )
        return rs_smb100a.attenuation;

    att =   rs_smb100a.attenuation + rs_smb100a_get_att_from_table( freq )
    ...
    if ( att > rs_smb100a.min_attenuation )
    {
        print( SEVERE, "Attenuation dynamic range is insufficient (f = "
               "%g MHz), using %f db instead of %f db.\n",
               freq * 1.0e-6, rs_smb100a.min_attenuation, att );
...
unsigned int
rs_smb100a_get_mod_param( Var_T ** v,
                        double * dres,
                        int *    ires )
{
...
void
rs_smb100a_check_mod_ampl( double freq )
{
    size_t i;
    ...
    if (    rs_smb100a.mod_type == MOD_TYPE_AM
         || rs_smb100a.mod_type == MOD_TYPE_OFF )
        return;

    if ( rs_smb100a.mod_type == MOD_TYPE_FM )
    {
        ...
        if ( rs_smb100a.mod_ampl[ rs_smb100a.mod_type ] >
                                               fm_mod_ranges[ i ].upper_limit )
        {
            print( SEVERE, "Current FM modulation amplitude of %.1f kHz is "
                   ...
                   "it to %.1f kHz.\n",
                   rs_smb100a.mod_ampl[ rs_smb100a.mod_type ] * 1.0e-3,
                   freq * 1.0e-6,  fm_mod_ranges[ i ].upper_limit * 1.0e-3 );

            if ( FSC2_MODE == EXPERIMENT )
                rs_smb100a_set_mod_ampl( rs_smb100a.mod_type,
