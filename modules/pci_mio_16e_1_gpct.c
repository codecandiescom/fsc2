...
Var_T *
daq_start_continuous_counter( Var_T * v )
{
    long counter;
    NI_DAQ_INPUT source;
    int ret;
...
Var_T *
daq_start_timed_counter( Var_T * v )
{
    int counter;
    NI_DAQ_INPUT source;
    double interval;
...
Var_T *
daq_intermediate_count( Var_T * v )
{
    int counter;
    unsigned long count;
    int state;
...
static NI_DAQ_INPUT
pci_mio_16e_1_gpct_source( const char * tname,
                           const char * snippet )
{
...
void
ni_daq_two_channel_pulses( double delay,
