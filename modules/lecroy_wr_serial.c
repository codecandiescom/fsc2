...
#else
        sprintf( cmd, "F%1d:TRA?\r", ch - LECROY_WR_TA + 1 );
#endif
...
lecroy_wr_start_acquisition( void )
{
...
#if ! defined LECROY_WR_IS_XSTREAM
        snprintf( cmd, 100, "T%c:DEF EQN,'AVGS(C%1ld)',MAXPTS,%ld,SWEEPS,%ld\r",
                  ch - LECROY_WR_TA + 'A',
...
#if ! defined LECROY_WR_IS_XSTREAM
        sprintf( cmd, "T%c:HMAG 1;T%c:HPOS 5\r", ch - LECROY_WR_TA + 'A',
                 ch - LECROY_WR_TA + 'A' ) ;
#else
        sprintf( cmd, "F%1d:HMAG 1;F%1d:HPOS 5\r", ch - LECROY_WR_TA + 1,
                 ch - LECROY_WR_TA + 1 ) ;
#endif
...
#if ! defined LECROY_WR_IS_XSTREAM
        sprintf( cmd, "T%c:FRST\r", ch - LECROY_WR_TA + 'A' );
#else
        sprintf( cmd, "F%1d:FRST\r", ch - LECROY_WR_TA + 1 );
#endif
        to_send = strlen( cmd );
        if ( fsc2_serial_write( lecroy_wr.sn, cmd, to_send,
                                TIMEOUT_FROM_LENGTH( to_send ), SET )
    ...
    /* Reset the bits in the register that tells us if fresh data are
       available */
                    ...
                    double         * offset )
{
    ...
    if ( ch >= LECROY_WR_CH1 && ch <= LECROY_WR_CH_MAX )
        sprintf( cmd, "C%1d:WF? DAT1\r", ch - LECROY_WR_CH1 + 1 );
    else if ( ch >= LECROY_WR_M1 && ch <= LECROY_WR_M4 )
        sprintf( cmd, "M%d:WF? DAT1\r", ch - LECROY_WR_M1 + 1 );
    else if ( ch >= LECROY_WR_TA && ch <= LECROY_WR_MAX_FTRACE )
#if ! defined LECROY_WR_IS_XSTREAM
        sprintf( cmd, "T%c:WF? DAT1\r", ch - LECROY_WR_TA + 'A' );
#else
        sprintf( cmd, "F%1d:WF? DAT1\r", ch - LECROY_WR_TA + 1 );
#endif
    else
        fsc2_impossible( );
 ...
 * (with the most sinificant byte in the first two chars)
 *---------------------------------------------------------------------*/

static int
lecroy_wr_s_hex_to_int( unsigned char * dp )
{
...
lecroy_wr_s_hex_to_nibble( unsigned char c )
{
    if ( isdigit( c ) )
        return c - '0';
...
#else
        sprintf( cmd, "F%1d:INSP? '%s'\r", ch - LECROY_WR_TA + 1, name );
#endif
    else
        fsc2_impossible( );
