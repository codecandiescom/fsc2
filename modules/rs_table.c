...
rs_find_table( char ** name )
{
    ...
    if ( ( tfp = rs_open_table( *name ) ) != NULL )
    {
        rs->table.file = T_strdup( *name );
        return tfp;
    }
...
double
rs_get_att_from_table( double freq )
{
    ...
    if ( freq < rs->table.min_freq )
    {
        print( WARN, "Frequency of %f MHz not covered by table, "
               "interpolating.\n", freq * 1.0e-6 );
        return rs->table.att_table[ 0 ].att;
    }
    ...
    if ( freq > rs->table.max_freq )
    {
        print( WARN, "Frequency of %f MHz not covered by table, "
               "interpolating.\n", freq * 1.0e-6 );
        return rs->table.att_table[ rs->table.len - 1 ].att;
    }
    ...
    i_low = 0;
    i_high = rs->table.len - 1;

    if ( freq == rs->table.att_table[ i_low ].freq )
        return rs->table.att_table[ i_low ].att;
    if ( freq == rs->table.att_table[ i_high ].freq )
        return rs->table.att_table[ i_high ].att;

    i_cur = lrnd( floor(   ( i_high - i_low )
                         * ( freq - rs->table.att_table[ i_low ].freq )
                         / (   rs->table.att_table[ i_high ].freq
                             - rs->table.att_table[ i_low ].freq ) ) ) + i_low;

    if ( freq > rs->table.att_table[ i_cur ].freq )
    {
        i_low = i_cur;
        if ( freq < rs->table.att_table[ i_cur + 1 ].freq )
            i_high = i_cur + 1;
    }
    else
        i_high = i_cur;
    ...
    att =   (   rs->table.att_table[ i_high ].att
              - rs->table.att_table[ i_low ].att )
          * ( freq - rs->table.att_table[ i_low ].freq )
          / (   rs->table.att_table[ i_high ].freq
              - rs->table.att_table[ i_low ].freq )
          + rs->table.att_table[ i_low ].att;
...
double
rs_get_att( double freq )
{
    double att;

    if ( ! rs->table.use_table )
        return rs->table.attenuation;

    att =   rs->table.attenuation + rs_get_att_from_table( freq )
    ...
    if ( att > rs->table.min_attenuation )
    {
        print( SEVERE, "Attenuation dynamic range is insufficient (f = "
               "%g MHz), using %f db instead of %f db.\n",
               freq * 1.0e-6, rs->table.min_attenuation, att );
        att = rs->table.min_attenuation;
    }
...
rs_check_mod_ampl( double freq )
{
    ...
    if (    rs->table.mod_type == MOD_TYPE_AM
         || rs->table.mod_type == MOD_TYPE_OFF )
        return;

    if ( rs->table.mod_type == MOD_TYPE_FM )
    {
        ...
        if ( rs->table.mod_ampl[ rs->table.mod_type ] >
                                               fm_mod_ranges[ i ].upper_limit )
        {
            print( SEVERE, "Current FM modulation amplitude of %.1f kHz is "
                   ...
                   "it to %.1f kHz.\n",
                   rs->table.mod_ampl[ rs->table.mod_type ] * 1.0e-3,
                   freq * 1.0e-6,  fm_mod_ranges[ i ].upper_limit * 1.0e-3 );

            if ( FSC2_MODE == EXPERIMENT )
                rs_set_mod_ampl( rs->table.mod_type,
                                 rs->table.mod_ampl[ rs->table.mod_type ] );
